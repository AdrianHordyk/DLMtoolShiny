...
library(DLMtool) # use functions from package

## Debugging Functions ##
as.list.DLM_fease=function(from) mapply(function(x) slot(from,x),
                                      slotNames("DLM_fease"),
                                      SIMPLIFY=FALSE)
...
# Modify data object and test 

# slotNames(DLMData)
# DLMData@MPeff <- 1
# DLMData@LHYear <- 0 
# DLMData@Year <- c(1:50)
# DLMData@ML <- array(runif(max(DLMData@Year), 90, 130), dim=c(1,max(DLMData@Year)))
# LstepCE1(x, DLMData)
 # save(DLMData, file="DLMData.Rdata") # Load data object
...
for (X in 3:length(slotNames(AllFeaseObj)))  
  slot(AllFeaseObj, slotNames(AllFeaseObj)[X]) <- 1	
